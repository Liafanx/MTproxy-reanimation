...
show_zapret2_menu() {
    while true; do
        ...
        if [ "${ZAPRET2_APPLIED:-false}" = "true" ]; then
            ...
            echo -e "  ${GREEN}[1]${NC}  Переустановить zapret2"
            ...
            # Не установлен
            echo -e "  ${GREEN}[1]${NC}  Установить zapret2"
...
show_zapret2_settings_menu() {
    while true; do
        ...
        case "$_choice" in
            ...
            7)
                echo -en "  fwmark [${ZAPRET2_FWMARK}] (например 0x40000000): "
                local _v; read -r _v
                if [ -n "$_v" ]; then
                    ZAPRET2_FWMARK="$_v"
                    save_settings
