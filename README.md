...
Сделал свой Прокси менеджер ставится в докере, вся установка автоматическая, образ есть в GitHub, управление через tui меню со встроенным syn limiter (реаниматором), версия 1.1.3 https://github.com/Liafanx/MTProxyL
...
<details>
<summary><b>1.2.0 от 25.07.2026</b></summary>

### Zapret2 MTProto fix by CHKRON

Добавлен новый режим обхода — **Zapret2 MTProto fix by CHKRON**.

Это принципиально другой подход по сравнению с SYN limiter:

- **SYN limiter** (Smart By-MEKO / Classic) — ограничивает количество входящих SYN-пакетов от клиента. Работает как защита от спама подключений и помогает клиенту быстрее "выбрать" рабочий путь.
- **Zapret2 fix by CHKRON** — работает на уровне TCP-пакетов и активно запутывает при анализе первого handshake.
...
4. Видно неполный/перепутанный ClientHello и не получается его проанализировать.
...
Клиенты iOS имеют специфический TCP fingerprint в SYN-пакете. Они автоматически определяются и пропускаются без window rewrite — это предотвращает деградацию подключения iOS-клиентов.
...
|----------|:---:|---------|
| `out-range` | `-s1` | Диапазон исходящих пакетов сервера для обработки |
| `split len` | `400` | Размер каждой из 3 частей разрезания |
| `win SYN+ACK` | `1400` | TCP window в SYN+ACK |
| `win ACK` | `10` | TCP window в пустых ACK |

#### Управление out-range
...
- Работает с любым Telemt / MTProxyL / MTProxyMax / Docker (нет тестов в режиме bridge), и в теории с другими Mtproto
...
- Поддерживает `public_host` и `public_port` из секции `[general.links]` конфига Telemt:
  - если они заданы — подставляет их в ссылки вместо IP сервера
  - если не заданы — ссылки выводятся как есть из API
- Работает через `jq` (добавлен в автоустановку зависимостей, отдельно ставить не нужно)
- API порт определяется автоматически из секции `[server.api]` конфига (по умолчанию `9091`)

Пример вывода:

```
  osnova  [вкл]  подключений: 2  уник. IP: 1
...
Добавлен новый пункт меню `[p] Параметры Telemt` — полное управление параметрами `client_mss`, `client_mss_bulk`, прямо из меню реаниматора без ручного редактирования конфига.

Доступные действия:
- Установить значение (`client_mss`, `client_mss_bulk`)
- Отключить отдельный параметр (строка комментируется)
- Отключить все три параметра сразу
- Включить MSS-пресет: `client_mss = tspu` + `mss_bulk = 1400`
- После изменений предлагается перезапустить Telemt

> ⚠ При **Smart By-MEKO** режиме `client_mss` не рекомендуются — скрипт предупреждает об этом в меню.

---

### Проверка ограничений сервера

Добавлен новый пункт меню `[x] Проверка ограничений сервера` — запускает внешний инструмент проверки доступности ресурсов с сервера через `censorcheck.tlab.pw`.

---

### Статус Telemt и параметры MSS в шапке

В шапку главного меню добавлены:
- **Telemt сервис** — показывает работает или остановлен (поддерживает docker, systemd, mtproxymax)
- **client_mss / client_mss_bulk** — текущие значения из конфига (зелёным если задано, серым если отключено)

---

### Улучшена работа с конфигом Telemt

- Добавлены надёжные TOML-хелперы для чтения значений по секции (`_toml_get_raw_in_section`, `_toml_get_string_in_section`, `_toml_get_number_in_section`)
- API порт теперь определяется корректно из секции `[server.api]`, а не по первому вхождению `listen =` в файле
- `jq` добавлен в список автоустанавливаемых зависимостей
...
Если старый метод работал, а новый нет.
...
- Показывает статус сервиса Telemt и текущие значения MSS-параметров в шапке меню
...
- **Zapret2 MTProto fix by CHKRON** — серверный обход через packet mangling (disorder + badsum + window control)
...
- Используя любой вариант решения syn ограничений, необходимо убедиться в том, что домен, используемый для Fake TLS имеет поддержку постквантового гибридного алгоритма обмена ключами, сочетающего классическую эллиптическую кривую, проверить это вы можете с помощью моего ТГ бота: [@Sni_checker_bot](https://t.me/Sni_checker_bot) отправив ему домен. Если выбранный домен этого не поддерживает - с огромной вероятностью после попытки подключения с ios прилетит блокировка и подключение не удастся, будет вечное соединение:
- 🟢 сервер принимает X25519MLKEM768 - отлично.
- 🔴 PQ не поддерживается + Peer Temp Key = X25519 - iOS устройства не смогут подключиться.

- При **Smart режиме** `client_mss` в конфиге не нужен — лучше отключить через `[p] Параметры Telemt` если он задан.
...
- Управление лимитами Smart режима:
  - `Меню` → `[c] Настройки Smart режима`
- При **Classic режиме** `client_mss = tspu` тоже не работает в большинстве случаев — лучше закомментировать через `[p] Параметры Telemt`.
- Если у iOS клиентов после перезапуска приложение висит соединение при Smart режиме — попробуйте **Fix для iOS вариант 1** (keepalive). Его всегда можно мгновенно откатить.
...
- Ссылки прокси выводятся через `[l] Ссылки прокси` — API должен быть включён в конфиге Telemt (`[server.api] enabled = true`). Если в конфиге задан `public_host` — он будет подставлен в ссылки автоматически.
...
Альтернативный режим обхода, использующий активное манипулирование TCP-пакетами.
...
| **Совместимость** | все серверы | требует zapret2 (nfqws2) |
...
```bash
mtpr
# → [z] Zapret2 MTProto fix by CHKRON
# → [1] Установить
```
...
2. Пустые ACK идут с window=10 → клиент не торопится
3. Первый data-пакет (ClientHello) режется на 3 части:
   - 1-я часть → нормально
   - 3-я часть → нормально со смещением  
   - 2-я часть → с битой контрольной суммой (badsum)
4. ТСПУ не может собрать ClientHello → пропускает соединение
5. Клиент ретрансмитирует среднюю часть → соединение устанавливается штатно
6. Дальше трафик без вмешательства

### Автозапуск после перезагрузки

NFT-правила применяются автоматически при старте службы `mtpr-zapret2.service`. Перезагрузка сервера не требует ручного вмешательства.
...
|----------|:------:|---------|
| out-range | `-s1` | Диапазон обработки исходящих пакетов |
| split len | `400` | Размер частей при разрезании |
| win SYN+ACK | `1400` | TCP window в SYN+ACK |
| win ACK | `10` | TCP window в пустых ACK |
...
| Параметр | Описание |
|----------|-----------|
| `client_mss` | MSS для клиентских соединений. **Не рекомендуется при Smart режиме** |
| `client_mss_bulk` | MSS для bulk-трафика |

Доступные действия: установить значение, отключить (закомментировать), включить MSS-пресет (`client_mss=tspu` + `client_mss_bulk=1400`), отключить все три сразу.
...
> **Важно (Classic):** если в конфиге Telemt есть `client_mss = ...`, его нужно убрать перед использованием Варианта 2. Используйте `[p] Параметры Telemt` → Отключить client_mss.
...
### Zapret2 MTProto fix — NFT таблица

```bash
nft list table ip MTProto
```

### Zapret2 — статус службы
...
- При использовании **Fix для iOS вариант 2** (Classic):
  - уберите `client_mss = ...` из конфига Telemt через `[p] Параметры Telemt`
...
- **Zapret2 MTProto fix by CHKRON** — альтернатива SYN limiter для серверов под активной блокировкой. Включается через `Меню → [z]`. При включении SYN limiter автоматически отключается — два режима одновременно не используются.
- При использовании **Zapret2 fix** параметр `client_mss` в конфиге Telemt лучше отключить через `[p] Параметры Telemt`.
