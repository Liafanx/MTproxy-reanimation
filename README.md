...
- Для Docker bridge предлагает два режима:
  - **Простой режим** — правило только по порту
  - **Точный Docker-режим** — внутренний IP контейнера + watcher
- Применяет **per-client inbound SYN limiter** через nftables — два режима на выбор
...
# Остальные сверх лимита → мгновенный RST
tcp dport PORT tcp flags syn
  reject with tcp reset
```
...
| Пресет | Режим | Rate | Burst | Описание |
|--------|-------|------|-------|----------|
| **★ Smart** | Smart | iOS: 15/sec / Other: 54/min | 30 / 1 | Рекомендуется |
| **Жёсткий** | Classic | 1/second | 1 | Строгое ограничение |
| **Средний** | Classic | 1/second | 3 | Если жёсткий слишком строг |
| **Мягкий** | Classic | 2/second | 5 | Для серверов с большим числом клиентов |
