...
- Значения при iOS №1 можно редактировать
</details>
...
- Определяет нужный netfilter hook: `input` (host) или `forward` (bridge)
- Применяет **per-client inbound SYN limiter** через nftables
- Безопасно применяет тюнинг Telemt (бэкап перед изменениями)
...
- Если указан IP → правила применяются только к трафику на этот адрес
...
Ускоряет обнаружение мёртвых сокетов через `sysctl`. Значения настраиваются. Применять если клиенты iOS после закрытия сессии не могут переподключиться.

По умолчанию: `time=60, intvl=15, probes=3` → обнаружение за ~105 сек.

### Вариант 2 — MSS + redirect

Создаёт отдельный порт для iOS (по умолчанию `4443`) с MSS=92 и прозрачным редиректом на основной порт.

> **Важно:** при использовании варианта 2 убедитесь, что в конфиге Telemt **нет** параметра `client_mss`. Если есть — удалите его.

iOS-пользователям нужно заменить порт в ссылке. Secret и IP остаются прежними.

## Основные команды

```bash
mtpr                                                    # Открыть меню
systemctl status mtpr-syn-limit.service --no-pager      # Статус службы
nft list table inet telemt_limit                        # Текущие правила
watch -n 2 'nft list chain inet telemt_limit input'     # Счётчик дропов
```
...
Перед изменением `config.toml` создаётся бэкап (`*.mtpr-backup-*`).
...
Тюнинг Telemt **не откатывается автоматически** — см. разделы выше.

## Важно

- Скрипт **не трогает** конфиги Telemt Panel
- **Не является заменой** Telemt или MTProxyMax
- NFT-правила работают на уровне ядра
- При смене порта — перезапустите службу: `systemctl restart mtpr-syn-limit.service`
