...
Сделал свой Прокси менеджер ставится в докере, вся установка автоматическая, образ есть в GitHub, управление через tui меню со встроенным syn limiter (реаниматором), версия 1.0.11 https://github.com/Liafanx/MTProxyL
...
### Управление параметрами Telemt (MSS / synlimit)

Добавлен новый пункт меню `[p] Параметры Telemt` — полное управление параметрами `client_mss`, `mss_bulk`, `synlimit` прямо из меню реаниматора без ручного редактирования конфига.

Доступные действия:
- Установить значение (`client_mss`, `mss_bulk`, `synlimit`)
- Отключить отдельный параметр (строка комментируется)
- Отключить все три параметра сразу
- Включить MSS-пресет: `client_mss = 92` + `mss_bulk = 1200`
- После изменений предлагается перезапустить Telemt

> ⚠ При **Smart By-MEKO** режиме `client_mss` и `synlimit` не рекомендуются — скрипт предупреждает об этом в меню.
...
- **client_mss / mss_bulk / synlimit** — текущие значения из конфига (зелёным если задано, серым если отключено)
...
- Управляет параметрами конфига Telemt: `client_mss`, `mss_bulk`, `synlimit`
...
| `mss_bulk` | MSS для bulk-трафика |
| `synlimit` | Встроенный SYN limiter Telemt. **Не рекомендуется при использовании реаниматора** |

Доступные действия: установить значение, отключить (закомментировать), включить MSS-пресет (`client_mss=92` + `mss_bulk=1200`), отключить все три сразу.
